        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-4">
            {RIGHT_COLLAGE_ITEMS.map((item, idx) => (
              <div
                key={idx}
                className={`rounded-3xl p-3 bg-zinc-950 border ${item.bg} shadow-2xl hover:scale-[1.02] transition-transform duration-300 flex flex-col justify-between ${idx % 2 === 1 ? "sm:mt-8" : ""
                  }`}
              >
